...
def get_station_and_gmt(querystr):
	result = re.search('([+-]?[0-9.]+ *, *[+-]?[0-9.]+)', querystr)
	if result is not None:
		querystr = result.groups()[0]
	else:
		if isinstance(querystr, unicode):
			try:
				querystr = querystr.encode('utf8', 'replace')
			except:
				logging.warning('Cuidate la espalda tebex.')
				return None
...
def currently_raining(station):
	feed = urllib.urlopen("http://rss.wunderground.com/auto/rss_full/global/stations/"+station[station.rfind('.')+1:]+".xml").read()
	result = re.search('\| Conditions: ([^|]+)\|', feed).groups()[0]

	logging.debug(result)

	for value in ['Thunderstorm', 'Rain', 'Drizzle', 'Spray', 'Showers']:
		logging.debug(value)
		if re.search(value, result):
			logging.debug("Beat it, beat it!")
			return True
